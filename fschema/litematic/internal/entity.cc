...
namespace fschema::litematic::internal {
  ...
  [[nodiscard]] ParseResult<std::array<double, 3>>
    ...
    if (count != 3) {
      ...
      for (std::size_t i = 0; i < count; ++i) {
        auto skip_result = base::SkipPayload(reader, base::TagType::Double);
        if (!skip_result) {
  ...
  [[nodiscard]] ParseResult<std::array<float, 2>>
    ...
    if (elem_type != base::TagType::Float) {
      return std::unexpected(reader.Error(ParseError::Code::InvalidTagId));
    }
    if (count != 2) {
      for (std::size_t i = 0; i < count; ++i) {
        auto skip_result = base::SkipPayload(reader, base::TagType::Float);
        if (!skip_result) {
          return std::unexpected(skip_result.error());
        }
      }
  ...
  [[nodiscard]] ParseResult<Entity> ParseEntityCompound(
    ...
    for (;;) {
      ...
      if ((name == "id" || name == "Id") && *tag_result == base::TagType::String) {
        auto value = reader.ReadStringView();
        if (!value) { reader.pop_depth(); return std::unexpected(value.error()); }
        entity.id = *value;
      }
      else if (name == "Pos" && *tag_result == base::TagType::List) {
      ...
      else if (name == "Motion" && *tag_result == base::TagType::List) {
      ...
      else if (name == "Rotation" && *tag_result == base::TagType::List) {
