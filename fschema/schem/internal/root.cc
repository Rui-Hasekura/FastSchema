...
namespace fschema::schem::internal {
  ...
  [[nodiscard]] static ParseResult<void>
    ParseMetadata(base::ByteReader& reader, Schematic& out) {
    reader.push_depth();
  ...
  [[nodiscard]] static ParseResult<void>
    ...
    for (;;) {
      ...
      if (name == "Palette" &&
        ...
        *tag_result == base::TagType::ByteArray) {
  ...
  [[nodiscard]] ParseResult<void> ParseRoot(
    ...
    if (*root_tag !=
      static_cast<std::uint8_t>(base::TagType::Compound)) {
      return std::unexpected(
        reader.Error(ParseError::Code::InvalidTagId));
    }
