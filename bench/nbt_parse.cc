...
[[nodiscard]] std::vector<TestFile> LoadTestFiles() {
  ...
  for (const auto& entry : std::filesystem::directory_iterator(dir)) {
    ...
    if (path.extension() == ".litematic") continue;
    ...
    size_t max_out = std::min(raw_data.size() * 10, static_cast<size_t>(512 * 1024 * 1024));
...
int main(int argc, char* argv[]) {
  ...
  for (size_t i = 0; i < g_files.size(); ++i) {
    ...
    std::cout << "  Bytes Consumed: " << reader.pos() << " / " << tf.bytes.size() << "\n";
  ...
  for (size_t i = 0; i < g_files.size(); ++i) {
    benchmark::RegisterBenchmark(
      ("PureNbtParse/" + g_files[i].filename).c_str(),
      BM_PureNbtParse
    )->Arg(i)->Unit(benchmark::kMicrosecond);
  }
