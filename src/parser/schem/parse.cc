...
namespace fschema::parser::schem {
  ...
  namespace {
    ...
    [[nodiscard]] std::expected<Schematic, ParseError>
      ...
      Schematic schematic;
      schematic.arena = std::make_unique<Arena>();
      ...
      auto result = detail::ParseRoot(reader, schematic);
      if (!result) {
  ...
  [[nodiscard]] std::expected<Schematic, ParseError>
    ...
    auto decompressed = DecompressGzipFile(path);
    if (!decompressed) {
