...
namespace fschema::parser::schem::detail {
  ...
  // Scalar varint decoder (unchanged)
  [[nodiscard]] inline ParseResult<std::uint32_t>
    DecodeOneVarint(
      const std::uint8_t*& p,
      const std::uint8_t* end) {
    if (p >= end) [[unlikely]] {
      return std::unexpected(ParseError{
          ParseError::Code::Truncated, "BlockData", 0 });
    }
  ...
  namespace {
    ...
    [[nodiscard]] std::pair<const std::uint8_t*, std::uint64_t>
      FindVarintBoundary(
    ...
    [[nodiscard]] std::pair<std::vector<std::uint64_t>, std::size_t>
      BuildVarintCumulativeCountParallel(
        std::span<const std::byte> data) {
      ...
      const std::size_t num_parts = std::min(
        n / (kGranularity * kMinPartBlocks),
      ...
      for (std::size_t t = 0; t < num_parts; ++t) {
        ...
        parts[t].byte_count = (t == num_parts - 1)
          ? n - t * part_size
          : part_size;
      }
  ...
  [[nodiscard]] ParseResult<NoInitVector<std::uint16_t>>
    ...
    const std::size_t num_chunks = std::min(
      static_cast<std::size_t>(n / 65536),
    ...
    for (std::size_t t = 0; t < num_chunks; ++t) {
      std::uint64_t target =
        static_cast<std::uint64_t>(t) * blocks_per_chunk;
      if (t > 0) {
      ...
      chunks[t].ptr = ptr;
      chunks[t].block_offset = static_cast<std::size_t>(count);
    }

    for (std::size_t t = 0; t < num_chunks - 1; ++t) {
      chunks[t].block_count =
        chunks[t + 1].block_offset - chunks[t].block_offset;
    }
    chunks[num_chunks - 1].block_count =
      n - chunks[num_chunks - 1].block_offset;
    ...
    tbb::parallel_for(
      ...
      [&](const tbb::blocked_range<std::size_t>& range) {
        for (std::size_t t = range.begin(); t < range.end(); ++t) {
          const std::uint8_t* p = chunks[t].ptr;
          std::uint16_t* dst = out.data() + chunks[t].block_offset;
          const std::size_t cnt = chunks[t].block_count;

          const bool aligned =
            (chunks[t].block_offset % 16) == 0;
          ...
          if (aligned && cnt >= 16) [[likely]] {
            ...
            for (std::size_t i = 0; i < aligned_cnt; ++i) {
              if ((i & 31) == 0) [[unlikely]] {
                _mm_prefetch(
                  reinterpret_cast<const char*>(p + 256),
                  _MM_HINT_T0);
              }
              ...
              if (buf_count == 16) {
                __m256i v = _mm256_load_si256(
                  reinterpret_cast<const __m256i*>(buf16));
                _mm256_stream_si256(
                  reinterpret_cast<__m256i*>(dst + i - 15),
                  v);
                buf_count = 0;
              }
            }
    ...
    if (err != 0) [[unlikely]] {
      return std::unexpected(ParseError{
          err == 1 ? ParseError::Code::Truncated
                   : ParseError::Code::PaletteIndexOutOfRange,
          "BlockData", 0 });
    }
  ...
  [[nodiscard]] ParseResult<NoInitVector<std::uint16_t>>
    ...
    if (data.size() < volume) [[unlikely]] {
      return std::unexpected(ParseError{
          ParseError::Code::BlockDataTooSmall,
          "BlockData", 0 });
    }
    if (data.size() == volume) [[likely]] {
      auto result = DecodeSingleByteFast(
        data, volume, palette_size, arena);
      if (result) {
        return result;
      }
      if (result.error().code !=
        ParseError::Code::VarintOverflow) {
        return std::unexpected(result.error());
      }
    }
    return DecodeVarintParallel(
      data, volume, palette_size, arena);
  }
