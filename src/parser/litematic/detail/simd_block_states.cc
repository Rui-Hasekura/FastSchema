...
namespace fschema::parser::litematic::detail {
  namespace HWY_NAMESPACE {
    ...
    [[nodiscard]] ParseResult<void> UnpackKernel(
      DTag d_tag, std::span<const std::uint64_t> longs,
      ...
      HWY_ALIGN std::uint64_t shifts_arr[8];
    ...
    [[nodiscard]] inline std::uint32_t ExtractBlock(
      ...
      std::size_t long_count, std::uint32_t bits_per_block,
      std::uint64_t block_idx, std::uint64_t bit_mask) {
    ...
    [[nodiscard]] ParseResult<void> UnpackFusedKernelAvx2(
      ...
      const __m256i vshifts = _mm256_setr_epi64x(
        0, static_cast<std::int64_t>(bits_per_block),
        static_cast<std::int64_t>(2) * bits_per_block,
        static_cast<std::int64_t>(3) * bits_per_block);
      const __m256i vmask =
        _mm256_set1_epi64x(static_cast<std::int64_t>(bit_mask));
      ...
      auto process_range = [&](std::uint64_t range_begin,
        std::uint64_t range_end) {
          ...
          auto GetWindow = [&](std::uint64_t bi, const std::uint64_t* longs,
          ...
          for (std::uint64_t iter_idx = range_begin; iter_idx < range_end;
            ...
            const __m256i unpacked_vec0 = _mm256_and_si256(
              _mm256_srlv_epi64(
                _mm256_set1_epi64x(static_cast<std::int64_t>(w0)), vshifts),
              vmask);
            ...
            const __m256i unpacked_vec1 = _mm256_and_si256(
              _mm256_srlv_epi64(
                _mm256_set1_epi64x(static_cast<std::int64_t>(w1)), vshifts),
              vmask);
            ...
            const __m256i unpacked_vec2 = _mm256_and_si256(
              _mm256_srlv_epi64(
                _mm256_set1_epi64x(static_cast<std::int64_t>(w2)), vshifts),
              vmask);
            ...
            const __m256i unpacked_vec3 = _mm256_and_si256(
              _mm256_srlv_epi64(
                _mm256_set1_epi64x(static_cast<std::int64_t>(w3)), vshifts),
              vmask);
            vmax3 = _mm256_max_epu32(vmax3, unpacked_vec3);
            const __m256i packed_vec3 =
              _mm256_permutevar8x32_epi32(unpacked_vec3, vperm);

            const __m128i pack01 = _mm_packus_epi32(
              _mm256_castsi256_si128(packed_vec0),
              _mm256_castsi256_si128(packed_vec1));
            // pack01 = 8xuint16

            const __m128i pack23 = _mm_packus_epi32(
              _mm256_castsi256_si128(packed_vec2),
              _mm256_castsi256_si128(packed_vec3));
            // pack23 = 8xuint16

            // sink -> 256-bit (16 x uint16 = 32 bytes)
            const __m256i result = _mm256_inserti128_si256(
              _mm256_castsi128_si256(pack01), pack23, 1);

            _mm256_stream_si256(
              reinterpret_cast<__m256i*>(out + block_idx), result);
          }
          ...
          const __m256i v_palette_signed =
            _mm256_xor_si256(v_palette, sign_flip);
          const __m256i max_vec_signed =
            _mm256_xor_si256(max_vec, sign_flip);
          const __m256i cmp =
            _mm256_cmpgt_epi32(v_palette_signed, max_vec_signed);
      ...
      for (std::uint64_t tail_idx = n_simd; tail_idx < volume; ++tail_idx) {
        const auto idx = ExtractBlock(raw_data, long_count, bits_per_block,
          tail_idx, bit_mask);
        if (idx >= palette_size) [[unlikely]] {
    ...
    [[nodiscard]] ParseResult<void> UnpackFusedKernelHwy(
      ...
      HWY_ALIGN std::uint64_t shift_arr[8];
      for (std::size_t lane_idx = 0; lane_idx < lanes; ++lane_idx) {
        shift_arr[lane_idx] = std::uint64_t(lane_idx) * bits_per_block;
      }
      ...
      auto Window = [&](std::uint64_t block_idx) -> std::uint64_t {
        ...
        const auto bit_shift = static_cast<std::uint32_t>(bit_offset & 63);
        const std::uint64_t high_bits =
          (bit_shift == 0) ? std::uint64_t(0)
          : next_long << (64 - bit_shift);
      ...
      for (std::uint64_t block_idx = 0; block_idx < n_simd;
        block_idx += lanes) {
    ...
    ParseResult<NoInitVector<std::uint16_t>> UnpackIndicesFusedImpl(
      ...
      if (long_count < min_longs) {
        return std::unexpected(
          make_error(ParseError::Code::BlockStatesTooSmall));
      }
      if (palette_size == 0) {
        return std::unexpected(
          make_error(ParseError::Code::PaletteIndexOutOfRange));
      ...
      NoInitVector<std::uint16_t> out(
        ...
        nbt::NoInitAllocator<std::uint16_t>{&arena});
      ...
      if (bits_per_block >= 2 && bits_per_block <= 16) {
        ...
        kernel_result = UnpackFusedKernelAvx2(
          raw_data, long_count, bits_per_block, volume, palette_size,
          out.data());
#else
        const hn::CappedTag<std::uint64_t, 4> d_tag;
        kernel_result = UnpackFusedKernelHwy(
          d_tag, raw_data, long_count, bits_per_block, volume, palette_size,
          out.data());
#endif
      }
      else {
        kernel_result = std::unexpected(
          make_error(ParseError::Code::PaletteIndexOutOfRange));
      }
    ...
    ParseResult<NoInitVector<std::uint16_t>> UnpackIndicesHwyImpl(
      ...
      if (bits_per_block >= 1 && bits_per_block <= 8) {
        const hn::ScalableTag<std::uint64_t> d_tag;
        ...
        return UnpackIndicesScalar(longs, bits_per_block, volume,
          palette_size);
      }
    }
